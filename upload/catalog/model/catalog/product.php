...
class ModelCatalogProduct extends Model {
	...
	/**
	 * Get verbose product data for getProducts method without SQL-query for each item
	 *
	...
	private function getProductList(&$products) {
		...
		foreach ($query->rows as $row) {
			$row = array_merge($row, $products[$row['product_id']]);
	...
	public function getProducts($data = array()) {
		...
		return $this->getProductList($product_data);
	}
	...
	public function getPopularProducts($limit) {
		...
		$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.viewed DESC, p.date_added DESC LIMIT " . (int)$limit);
	...
	public function getProductAttributes($product_id) {
		...
		$query = $this->db->query("SELECT ag.attribute_group_id, agd.name AS gname, a.attribute_id, ad.name AS aname, pa.text
			...
			ORDER BY ag.sort_order, ag.attribute_group_id, agd.name, a.sort_order, ad.name");
		foreach ($query->rows as $arow) {
			if (!isset($product_attribute_group_data[(string)$arow['attribute_group_id']])) {
				$product_attribute_group_data[(string)$arow['attribute_group_id']] = array(
					'attribute_group_id' => $arow['attribute_group_id'],
					'name'               => $arow['gname'],
					'attribute'          => array()
				);
			}
			$product_attribute_group_data[(string)$arow['attribute_group_id']]['attribute'][] = array(
